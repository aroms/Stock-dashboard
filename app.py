...
def signal_summary(close, rsi_series, macd_line, macd_signal, sma_fast, sma_slow):
    ...
    if len(macd_line) >= 2 and not np.isnan(macd_line.iloc[-1]) and not np.isnan(macd_signal.iloc[-1]):
...
with st.sidebar.expander("Backtest (SMA crossover)"):
    ...
    initial_capital = st.number_input("Initial capital ($)", min_value=1000, max_value=1_000_000, value=10_000, step=500)
...
if "Close" not in data.columns:
    # try to find Close_* if MultiIndex flattened earlier
...
with tab_chart:
    ...
    with col1:
        ...
        fig_rsi.add_hrect(y0=30, y1=30, line_width=0)
        fig_rsi.add_hrect(y0=70, y1=70, line_width=0)
...
with tab_backtest:
    ...
    fig_bt.add_trace(go.Scatter(x=equity.index, y=(1+returns).cumprod()* (initial_capital / ((1+returns).cumprod().iloc[0] if len(returns) else 1)), mode='lines', name='Buy & Hold'))
...
with tab_portfolio:
    ...
    if st.button("Run Portfolio Simulation"):
        ...
        # Download
        panel = yf.download(tickers, start=pf_start, end=pf_end, interval=pf_interval, auto_adjust=True, progress=False)["Close"]
        ...
        for col in panel.columns:
            fig_pf.add_trace(go.Scatter(x=(1+returns[col]).cumprod().index, y=(1+returns[col]).cumprod(), mode='lines', name=col, opacity=0.5))
...
st.markdown("---")
st.caption("Built with Streamlit • Data from Yahoo Finance via yfinance • Educational use only")
